...
const InternalButton: React.ForwardRefRenderFunction<HTMLButtonElement, ButtonProps> = (props, ref) => {
    ...
    if (variant === 'link') {
        return <AnchorStyled href={href} className={className} target={target} style={style} variant="link" size={size}>{children}</AnchorStyled>
    }

    return (
        <ButtonStyled ref={ref} className={className} style={style} variant={variant} size={size} disabled={disabled} onClick={handleClick}>{children}</ButtonStyled>
    )
}
